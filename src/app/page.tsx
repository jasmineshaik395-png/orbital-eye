...
import { addSyntheticFlights } from '@/lib/synthetic-flights';
...
export default function Dashboard() {
  ...
  const data = useMemo(() => addSyntheticFlights(dataRef.current), [dataVersion]);

  const [backendStatus, setBackendStatus] = useState<'connecting' | 'connected' | 'error'>('connecting');
  const [mapView, setMapView] = useState({ zoom: 0.8, latitude: 20 });
      ...
      fetch('/api/geo', { signal: geoController.signal })
        ...
        .then(geo => {
          if (!autoLocateCancelled.current && !geoController.signal.aborted && geo.status === 'success' &&
            ...
            setFlyToLocation({ lat: geo.lat, lng: geo.lon, zoom: 0.8, ts: Date.now() });
          }
        })
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      if (e.key === 'r' && !e.ctrlKey && !e.metaKey) setFlyToLocation({ lat: 20, lng: 0, zoom: 0.8, ts: Date.now() });
      if (e.key === 'g') {
        setActiveLayers(prev => ({ ...prev, terrain_elevation: false, terrain_3d: false }));
        ...
        {showIntro && (
          ...
          >
            ...
            <button
              ...
              className="absolute bottom-6 right-6 rounded border border-white/30 bg-black/60 px-4 py-2 text-[10px] font-mono tracking-[0.25em] text-white/80 hover:bg-black/80"
            >
              SKIP INTRO
            </button>
          </motion.div>
